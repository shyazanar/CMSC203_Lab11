...
public interface OptimizerManagerInterface {
	...
	/**
	 * The optimize method iterates from the left extent to the right extent,
	 * computing the function at each point, and saving the minimum or maximum
	 * values according to the optimization problem being estimated, and saving the
	 * x values that produce them.
	 * 
	 * @param choice the problem number to be optimized
	 * @return an instance of the Optimum class, containing the optimal value and
	 *         the x value that produced it, along with y and z values.
	 */
	public Optimum optimize(int choice);

	/**
	 * getFnValue calculates the value of the function requested by the user, at a
	 * specific x value.
	 * 
	...
	/**
	 * Sets the left and right extents to be considered, and computes and sets the
	 * minimum and maximum values of f(x) for those left-right extents. Note that
	 * these values are NOT transformed to fit in the display grid, they are just
