...
public class MainPaneFX {
	...
	/**
	 * The MainPanel constructor sets up the GUI with two buttons and a display
	 * area.
	 */
	MainPaneFX(double CANVAS_WIDTH, double CANVAS_HEIGHT) {
		...
		/*
		 * When the button pushed Optimize button, user is prompted to select one of the
		 * problems to optimize, and is asked for the left and right limits (extents) to
		 * plot the function.
		 */
		optimizeButton.setOnAction(event -> {
			...
			Optimum opt = null;
			choice = askForFunction(
					"Select one of the following problems to optimize (by number):\n" + optimizerManager.toString());
			if (choice != 0) {
...
//					JOptionPane.showMessageDialog(null, optimizerManager.answerString(choice, opt.getOptimum(),
//							opt.getOptX(), opt.getOptY(), opt.getOptZ()));
					new Alert(Alert.AlertType.INFORMATION, optimizerManager.answerString(choice, opt.getOptimum(),
							opt.getOptX(), opt.getOptY(), opt.getOptZ())).showAndWait();
		...
		do {
			try {
				...
				if (returnVal < 1 || returnVal > 3) {
					...
					new Alert(Alert.AlertType.ERROR, "Choice of problem must be an integer between 1 and 3").showAndWait();
				} else
					error = false;
